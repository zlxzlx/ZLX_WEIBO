...
@implementation ZLXEmotionTextView
- (void) appendEmotion:(ZLXEmotionModel *)emotion{
    if (emotion.emoji) { // emoji表情
        ...
        // 创建一个带有图片表情的富文本
        ZLXEmotionAttachment *attach = [[ZLXEmotionAttachment alloc] init];
...
//    1.用来拼接所有文字
        ...
        [self.attributedText enumerateAttributesInRange:NSMakeRange(0, self.attributedText.length) options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            ZLXEmotionAttachment *attach = attrs[@"NSAttachment"];
